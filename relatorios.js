...
document.getElementById('applyFilter').addEventListener('click', () => {
    ...
    const reportsRef = window.firebaseDatabase.ref('reports/' + motoboyEmail.replace(/\./g, '_'));
...
window.deleteReport = function(reportId) {
    const motoboyEmail = document.getElementById('motoboy').value;
    const reportRef = window.firebaseDatabase.ref('reports/' + motoboyEmail.replace(/\./g, '_') + '/' + reportId);
    reportRef.remove()
        .then(() => {
